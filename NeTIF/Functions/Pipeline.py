...
def pipeline(in_dir,out_dir,dataset,processed_csv_file, just_label=False):
    ...
    if dataset=='UNSW':
        ...
        if not check:
            ...
            # 'just_label' check, added by Stephen
            ...
            if not just_label:
                ...
                pcap_file_list = glob.glob(os.path.join(pcap_feb, "*.pcap"))
                pcap_file_list.sort()  # Optional: ensures files are in numerical order like 1.pcap, 2.pcap, etc.

                print(f"[INFO] Found {len(pcap_file_list)} PCAP files in {pcap_feb}")
                if not pcap_file_list:
                    logging.warning("No PCAP files found! Please check your directory path.")
                ...
                csv_files = glob.glob(os.path.join(out_file, "pcap_csv_*.csv"))
                csv_files.sort()  # Optional: sort by filename

                for csv_path in csv_files:
                    if os.path.exists(csv_path):
                        pcap_csv.append(csv_path)
                    else:
                        logging.warning(f"Expected CSV not found: {csv_path}")
                ...
                logging.info("Detecting 17-2-2015 Files .........")
                pcap_file_list = []
                pcap_feb = in_dir + "/pcap 17-2-2015/"
