...
def label(pcap_csv,UNSW_csv,output_file,file_num):
    ...
    for pcap_file in pcap_csv:
        ...
        # Force all join columns to have the same dtype
        # join_cols = ['stime', 'ltime', 'srcip', 'dstip', 'dsport', 'sport', 'sttl', 'protocol_m']

        # df_pcap_csv['stime'] = df_pcap_csv['stime'].astype(int)
        # df_pcap_csv['ltime'] = df_pcap_csv['ltime'].astype(int)
        # df_pcap_csv['dsport'] = df_pcap_csv['dsport'].astype(int)
        # df_pcap_csv['sport'] = df_pcap_csv['sport'].astype(int)
        # df_pcap_csv['sttl'] = df_pcap_csv['sttl'].astype(int)
        # df_pcap_csv['protocol_m'] = df_pcap_csv['protocol_m'].astype(str)
        #
        # df_red['stime'] = df_red['stime'].astype(int)
        # df_red['ltime'] = df_red['ltime'].astype(int)
        # df_red['dsport'] = df_red['dsport'].astype(int)
        # df_red['sport'] = df_red['sport'].astype(int)
        # df_red['sttl'] = df_red['sttl'].astype(int)
        # df_red['proto'] = df_red['proto'].astype(str)

        # print("==== dtypes in df_pcap_csv ====")
        # print(df_pcap_csv.dtypes)
        # print("\n==== dtypes in df_red ====")
        # print(df_red.dtypes)
        #
        # print("\n==== sample df_pcap_csv ====")
        # print(df_pcap_csv.head())
        #
        # print("\n==== sample df_red ====")
        # print(df_red.head())

        # Convert columns in df_red to match df_pcap_csv
        def safe_int(x):
            try:
        ...
        df_red['dsport'] = df_red['dsport'].apply(safe_int)
        df_red['sport'] = df_red['sport'].apply(safe_int)

        df_red['proto'] = df_red['proto'].astype(str)

        # Ensure protocol_m is also string in pcap side
        df_pcap_csv['protocol_m'] = df_pcap_csv['protocol_m'].astype(str)
